...
public class FeatureModelResourceVistor implements ResourceClient {
	...
	private void proccessClassAnnotations(IResource resource) {	
		...
		//Adicionando a feature core, sobre a qual os artefatos não mapeados serão associados.
		if(featuresContent.getFeatureItens().isEmpty()){
			FeatureItem coreFeature = new FeatureItem();
			coreFeature.setName("Core");
			coreFeature.setParent(resource.getProject().getName());
			coreFeature.setType(FeatureType.mandatory);
			featuresContent.getFeatureItens().add(coreFeature);
		}
		
		//Adicionando features anotadas em CodePieces
		File tempoFile = new File(resource.getLocationURI().getPath());
		BufferedReader in;
		try {
			in = new BufferedReader(new FileReader(tempoFile));
			...
			featuresContent.getFeatureItens().addAll(parserAnnotationsCodePieces(text));
	...
	private List<FeatureItem> parserAnnotationsCodePieces(String source) {
