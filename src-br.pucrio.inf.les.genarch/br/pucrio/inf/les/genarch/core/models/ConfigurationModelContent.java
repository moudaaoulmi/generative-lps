...
public class ConfigurationModelContent {
	...
	public boolean addClass(String name,String path,String featureConfiguration,String componentPath) {
	 ...
	 public void addField(String name,String fieldDeclaration, String path, ConfigurationClass configurationClass, String featureConfiguration) {
		 ConfigurationField configurationField = getField(name, path);
		 
		 if(configurationField != null){
			 configurationField.getFeatureExpression().setExpression( configurationField.getFeatureExpression().getExpression() + " and " + featureConfiguration);
		 }else{
			 FeatureExpression feature = configurationFactory.createFeatureExpression();
			 feature.setExpression(featureConfiguration);
			 configurationField = this.configurationFactory.createConfigurationField();
	 ...
	 public ConfigurationCodePiece addCodePiece(int start, int end, String path, MappingEntity parent, String featureConfiguration) {
		 if ( !containsMappingEntity(path) ) {
			FeatureExpression feature = configurationFactory.createFeatureExpression();
			feature.setExpression(featureConfiguration);
		 	ConfigurationCodePiece codePiece = this.configurationFactory.createConfigurationCodePiece();
	    	codePiece.setName("["+start+","+end+"]");
	    	codePiece.setPath(path);
	    	codePiece.setFeatureExpression(feature);
	    	if(parent instanceof ConfigurationMethod)
	    		((ConfigurationMethod)parent).getCodePieces().add(codePiece);
	    	else if(parent instanceof ConfigurationCodePiece)
	    		((ConfigurationCodePiece)parent).getCodePieces().add(codePiece);
	    	return codePiece;
	    }else {
	    	return getConfigurationCodePiece(path);
	    }
	 }
	 
	 
	 public ConfigurationMethod addMethod(String name,String methodDeclaration, String path, ConfigurationClass configurationClass, String featureConfiguration) {
		  ConfigurationMethod configurationMethod = getMethod(name, path);
		 
		 if(configurationMethod != null){
			 configurationMethod.getFeatureExpression().setExpression( configurationMethod.getFeatureExpression().getExpression() + " and " + featureConfiguration);
			 
		 }else{
			 FeatureExpression feature = configurationFactory.createFeatureExpression();
			 feature.setExpression(featureConfiguration);
			 configurationMethod = this.configurationFactory.createConfigurationMethod();
	...
	public ConfigurationCodePiece getConfigurationCodePiece(String path){

		final TreeIterator iterator = this.mappingRelationships.eAllContents();

		while ( iterator.hasNext() ) {
			final EObject o = (EObject)iterator.next();
			if ( o instanceof ConfigurationCodePiece ) {
				ConfigurationCodePiece c = (ConfigurationCodePiece)o;
				if ( c.getPath().equals(path) ) {
					return c;
				}
			}
	...
	public boolean containsMappingEntity(String path) {
		...
		while ( iterator.hasNext() ) {
			...
			} else if ( o instanceof ConfigurationCodePiece ) {
				ConfigurationCodePiece c = (ConfigurationCodePiece)o;
				if ( c.getPath().equals(path) ) {
					return true;
				}
	...
	public ConfigurationMethod getMethod(String name, String path) {
		final TreeIterator iterator = this.mappingRelationships.eAllContents();

		while ( iterator.hasNext() ) {
			final EObject o = (EObject)iterator.next();

			if ( o instanceof ConfigurationMethod ) {
				ConfigurationMethod c = (ConfigurationMethod)o;				
	...
	public ConfigurationField getField(String name, String path) {
		final TreeIterator iterator = this.mappingRelationships.eAllContents();
		...
		while ( iterator.hasNext() ) {
			...
			if ( o instanceof ConfigurationField ) {
				ConfigurationField c = (ConfigurationField)o;				
				if ( c. getName().equals(name) && c.getPath().equals(path) ) {
					return c;
				}
			}
